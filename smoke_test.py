...
def test_reviews():
    ...
    r = rows[0]
    ok &= check("review_id from data-reviewid", r.review_id == "RXN5D0N6KQ6KP")
    ok &= check("rating 5.0 from the review's own star node", r.rating == 5.0)
    ok &= check("author read", r.author == "Angela")
    ...
    ok &= check("accessibility filler stripped from the body",
                ...
                and (r.body or "").startswith("Very comfortable headphones!"))
    ok &= check("title has no rating text prefixed",
                r.title == "Great sound and very comfortable")
    ok &= check("url is the per-review permalink, not the product page",
                r.url == "https://www.amazon.com/gp/customer-reviews/RXN5D0N6KQ6KP")
